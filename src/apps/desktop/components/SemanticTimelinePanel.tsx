...
import { Video, Type, Mic, Scissors, MousePointer2, Crosshair, ZoomIn, ZoomOut } from 'lucide-react';
    ...
    <div className="h-[320px] bg-[#0a0a0c] border-t border-[#1a1a20] flex flex-col select-none">
      ...
      <div className="flex px-4 py-2 border-b border-[#1a1a20] shrink-0 justify-between items-center bg-[#070708]">
        <div className="flex gap-2">
          ...
          <button onClick={() => setZoomLevel(clampZoom(zoomLevel - 1))} className="flex items-center gap-1 px-2 py-1 rounded text-[10px] text-[#666] hover:bg-[#1a1a20] hover:text-[#aaa] transition-colors"><ZoomOut className="w-3 h-3" /></button>
          <span className="text-[10px] font-mono text-[#555] flex items-center">{zoomLevel.toFixed(1)}x</span>
          <button onClick={() => setZoomLevel(clampZoom(zoomLevel + 1))} className="flex items-center gap-1 px-2 py-1 rounded text-[10px] text-[#666] hover:bg-[#1a1a20] hover:text-[#aaa] transition-colors"><ZoomIn className="w-3 h-3" /></button>
        </div>
      ...
      <div ref={scrollContainerRef} onMouseMove={handleMouseMove} onClick={handleTimelineClick} className={`flex-1 relative overflow-x-auto overflow-y-hidden transition-opacity duration-300 ${isRebuilding ? 'opacity-50' : 'opacity-100'} ${activeTool === 'blade' ? 'cursor-crosshair' : ''}`}>
            ...
            <div className="relative h-10 w-full bg-[#111115] border-y border-[#1a1a20] group">
              ...
              {cuts.map((cut) => {
                ...
                return (
                  <div
                    key={`v-${cut.id}`}
                    onClick={(e) => { e.stopPropagation(); if (activeTool === 'blade') { const pct = getPctFromMouseEvent(e); splitCut(pct * totalDuration); setActiveTool('selection'); } else if (activeTool === 'selection') { setSelectedCutId(cut.id); } }}
