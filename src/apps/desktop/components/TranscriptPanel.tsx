...
export default function TranscriptPanel() {
  ...
  // 1. Calcula as correspondências (matches) da busca
  ...
  // 2. Reseta o índice de navegação quando a busca muda
  useEffect(() => {
    setCurrentMatchIdx(0);
  }, [searchQuery]);

  // 3. Auto-scroll suave para a palavra ativa da busca
  useEffect(() => {
    if (matches.length > 0) {
  ...
  const handlePrevMatch = () => {
    setCurrentMatchIdx(prev => (prev > 0 ? prev - 1 : matches.length - 1));
  };

  const handleNextMatch = () => {
    setCurrentMatchIdx(prev => (prev < matches.length - 1 ? prev + 1 : 0));
  };
      ...
      <div className="h-12 border-b border-[#1c1c20] flex items-center px-3 gap-2 flex-shrink-0 bg-[#070708]">
        <div className="flex-1 relative flex items-center">
          ...
          <input
            ...
            onKeyDown={(e) => {
               // Permite navegar dando "Enter" no teclado!
          ...
          {matches.length > 0 && (
            <div className="absolute right-1 flex items-center bg-[#111115] pl-2">
              ...
              <button onClick={handlePrevMatch} className="p-1 text-[#888] hover:text-[#eee] transition-colors rounded hover:bg-[#222]">
                <ChevronUp className="w-3.5 h-3.5" />
              </button>
              <button onClick={handleNextMatch} className="p-1 text-[#888] hover:text-[#eee] transition-colors rounded hover:bg-[#222]">
